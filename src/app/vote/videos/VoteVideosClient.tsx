...
export default function VoteVideosClient() {
  ...
  const loadData = async () => {
    try {
      const [assetsResponse, ballotResponse] = await Promise.all([
        fetch('/api/feed?type=video'),
        fetch('/api/ballot?category=VIDEO'),
      ])
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {assets.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No videos available for voting</p>
          </div>
        )}
